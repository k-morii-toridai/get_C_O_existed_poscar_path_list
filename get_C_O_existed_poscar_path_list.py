...
C_O_existed_poscar_file_path_list = poscar_abs_path_list_loaded[bool_C_O_exist_list]
print(f"len(C_O_existed_poscar_file_path_list): {len(C_O_existed_poscar_file_path_list)}")
np.save('C_O_existed_poscar_file_path_list.npy', C_O_existed_poscar_file_path_list)
# CとOを含むPOSCARファイルのフォルダのリストを.npy形式で保存
C_O_existed_poscar_folder_path_list = [Path(os.path.split(p)[0]) for p in C_O_existed_poscar_file_path_list]
np.save('C_O_existed_poscar_folder_path_list.npy', C_O_existed_poscar_folder_path_list)
print("C_O_existed-poscar file, and folder path list were saved as .npy!!!")
